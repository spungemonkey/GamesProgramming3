...
namespace Lab5
{
    ...
    public class Game1 : Microsoft.Xna.Framework.Game
    {
        ...
        GameState CurrentState = GameState.Play; //the gamestates for the game. Initialised the state to "play"
        ...
        // Camera Variables
        //--------------------------------------------------
        Camera freeCamera; //creates secondary camera
        Camera mainCamera; //creates the main camera. 
        ...
        {
            aspectRatio = graphics.GraphicsDevice.Viewport.AspectRatio; //sets the aspect ration
            ...
            mainCamera = new Camera(this, new Vector3(10.0f, 0.0f, -10.0f), Vector3.Zero, 5.0f); //initialises the main camera. This is initialised second so that it is the main camera
        ...
        private void gameControls()
        {
            ...
            if (keyboardState.IsKeyDown(Keys.Space) && lastState.IsKeyUp(Keys.Space) || GamePad.GetState(PlayerIndex.One).Buttons.A == ButtonState.Pressed) //fires the laser
            {
                DateTime startVib;
                TimeSpan endVib;
                ...
                for (int i = 0; i < GameConstants.NumBolts; i++)
                {
                    ...
                    if (!boltList[i].isActive) //creates the bolts if there are none active in the game
                    {
                        ...
                        boltList[i].speed = GameConstants.LaserSpeedAdjustment; //sets the speed of the bolt
                ...
                if (dfgList[i].isActive)
                {
                    ...
                    for (int k = 0; k < boltList.Length; k++) //loop for the bolts
                    {
                        if (boltList[k].isActive) //determines if a bolt is active
                        {
                            ...
                            if (dfgSphereA.Intersects(boltSphere)) //check for collision between a laser and the DFG
                            {
                                dfgList[i].isActive = false; //removes the DFG
                                boltList[k].isActive = false; //Removes the laser
                        ...
                        if (dfgSphereA.Intersects(ysSphere)) //Check collision between dfg and the player
                        {
                            ...
                            pHealth = pHealth - 2;//reuces player health
        ...
        protected override void Update(GameTime gameTime)
        {
            ...
            if (keyboardState.IsKeyDown(Keys.T) && lastState.IsKeyUp(Keys.T)) //command to toggle sound
            {
        ...
        protected override void Draw(GameTime gameTime)
        {
            ...
            spriteBatch.Draw(healthBar, new Rectangle(20, 470, (int)(200 * ((double)pHealth / 100)), 5), Color.Yellow); //draws the player healthbar at the bottom of the window
            ...
            switch (CurrentState)
            {
                case GameState.Play: //the following things are drawn during GameState.Play
                    ...
                    for (int i = 0; i < GameConstants.NumBolts; i++) //begin loop for bolts
                    {
                        ...
                        {
                            Matrix bTransform = Matrix.CreateScale(GameConstants.LaserScalar) * Matrix.CreateTranslation(boltList[i].position); //creates transform for bolts
                ...
                case GameState.Boss:
                    for (int i = 0; i < GameConstants.NumDFG; i++)
                    {
                        if (dfgList[i].isActive)
                        {
                            Matrix dalekTransform = Matrix.CreateRotationY(20) * Matrix.CreateScale(GameConstants.DFGScalar) * Matrix.CreateTranslation(dfgList[i].position);
                    ...
                    for (int i = 0; i < GameConstants.NumBolts; i++)
                    {
                        if (boltList[i].isActive)
                        {
                            Matrix laserTransform = Matrix.CreateScale(GameConstants.LaserScalar) * Matrix.CreateTranslation(boltList[i].position);
                            DrawModel(mdlBolt, laserTransform, boltTransforms);
                        }
                    }
            ...
            spriteBatch.End(); //ends sprite batch so text can draw
