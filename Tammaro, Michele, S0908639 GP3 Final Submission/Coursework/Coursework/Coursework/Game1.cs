...
namespace Lab5
{
    ...
    public class Game1 : Microsoft.Xna.Framework.Game
    {
        ...
        /*
        //--------------------------------------------------
        //EXTRA FEATURES IMPLEMENTED INTO THIS GAME
        //--------------------------------------------------

         Fog (with Toggle)
         Xbox 360 Controls
         Boss "AI" - The enemy boss moves randomly between the waypoints
         Multiple game themes that change depedning on state
         Terrain - Implemented a sky box
         User/Boss Energy - a health bar for both the player and the enemy boss
         Gamestates and a Menu system.
         */
        ...
        private Texture2D mainMenu;         //Image used to when Gamestate is in menu
        ...
        private Model bossBolt;                                                 //create model for the boss bolts
        private Matrix[] bossboltTransforms;                                    //set up a matrix to transform the bolt
        private Bolt[] bbList = new Bolt[GameConstants.NumBossBolts];           //creates an array of boss bolts from the class bolt
        ...
        private Vector3[] bossPoints = {
                                ...
                                };                                           //array of waypoints for the boss to travel between. These are defined in Game1.cs as they cannot be declared in a static class (GameConstants)

         
        private Bolt[] waypoints = new Bolt[GameConstants.NumWaypoints];    //creates a array of waypoints. Uses the Bolt class so that the waypoints can implement its methods
        ...
        // Game Variables
        //--------------------------------------------------
        private int count = 0;                                              //number of DFG that have been defeated by the enemy
        private int totalCount = 0;                                         //total number of DFG defeat by the enemy
        private int score = 0;                                              //score that will be written to external file
        private double gTimer = 100;                                        //amount of time the player has to play the game. equates to 100 seconds
        private Random random = new Random();
        private KeyboardState lastState;                                        //creates an instance of keyboard state
        private GamePadState lastGPState;                                       //create an instance of gamepad state
        private bool soundToggle = false;                                       //boolean used to toggle the songs in the game.
        private bool fogOn = true;                                              //boolean used to toggle fog
        StreamWriter scoreFile;                                                 //name of StreamWriter that writes the score
        ...
        private void gameControls()
        {
            ...
            GamePadState gamepadState = GamePad.GetState(PlayerIndex.One);                                   //current state of the gamepad
            ...
            if (keyboardState.IsKeyDown(Keys.Down) || 
                gamepadState.ThumbSticks.Left.Y <= -0.5f)                                                   //move player down
            {
                ysVelocityAdd *= 0.01f;
                ysVelocity += ysVelocityAdd;                                                                //adds velocity and moves the player
                GamePad.SetVibration(PlayerIndex.One, 0.1f, 0.0f);                                          //vibrates controller - low frequency
            ...
            if (keyboardState.IsKeyDown(Keys.Up) || 
                gamepadState.ThumbSticks.Left.Y >= 0.5f) //move player up
            {
                ...
                {
                    ysVelocityAdd *= 0.0f;                                  //set velocity to 0
                    ysVelocity *= ysVelocityAdd;                            //adds velocity and moves player
                }
            }
            ...
            {
                GamePad.SetVibration(PlayerIndex.One, 0.0f, 0.0f);          //stops vibration and movement when the thumbstick is at the deadzone
            }



            if (keyboardState.IsKeyDown(Keys.R) || 
                gamepadState.Buttons.Start == ButtonState.Pressed && lastGPState.Buttons.Start == ButtonState.Released)     //resets the game
                gameReset();


            if (keyboardState.IsKeyDown(Keys.Space) && lastState.IsKeyUp(Keys.Space) || 
                gamepadState.Buttons.A == ButtonState.Pressed && lastGPState.Buttons.A == ButtonState.Released)     //fires the bolt
            {
                ...
                for (int i = 0; i < GameConstants.NumBolts; i++)
                {
                    ...
                    {
                        Matrix boltTransform = Matrix.CreateRotationX(90);
                        boltList[i].direction = boltTransform.Left;                 //sets direction of the bolt, travels from left to right
                        ...
                        GamePad.SetVibration(PlayerIndex.One, 0.0f, 1.0f);          //controller vibrations again to increase lenth of vibration
                        break; //exit the loop     
                    }
                }
            }
            ...
            if (keyboardState.IsKeyDown(Keys.D1) && lastState.IsKeyUp(Keys.D1) || 
                gamepadState.Buttons.Y == ButtonState.Pressed && lastGPState.Buttons.Y == ButtonState.Released)                 //change the camera
            {
            ...
            if (keyboardState.IsKeyDown(Keys.F) && lastState.IsKeyUp(Keys.F) || gamepadState.Buttons.B == ButtonState.Pressed && lastGPState.Buttons.B == ButtonState.Released)
                fogOn = !fogOn;


            lastState = keyboardState;                      //sets lastState to Keyboardstate. Prevents input from happening 60 times a second
            lastGPState = gamepadState;                     //sets lastGPState to gamepadState. Prevents input from happening 60 times a second
        ...
        {
            Matrix[] absoluteTransforms = new Matrix[myModel.Bones.Count];                                    //creates a translation matrix that applies it to all bones in the model
            myModel.CopyAbsoluteBoneTransformsTo(absoluteTransforms);                                         //copies absolute bone transform to the matrix

            foreach (ModelMesh mesh in myModel.Meshes)                                                        //can have many meshes in a model, must applie effect to each
            {
                foreach (BasicEffect effect in mesh.Effects)                                                  //can have many effects in meshes. Must apply to all
                {
                    effect.EnableDefaultLighting();                                                           //enables default lights

                    if (myCam)                                                                                //each camera has different projection and view matrices. applies different proj/view matrices
                    {
                        effect.Projection = freeCamera.Projection;                                            //applies projection to effect
                        effect.View = freeCamera.View;                                                        //applies prohection to effect
                    }
                    else
                    {
            ...
            return absoluteTransforms;                                                                          //returns a translation matrix
        }
        ...
        public void DrawModel(Model model, Matrix modelTransform, Matrix[] absoluteBoneTransforms)
        {
            ...
            foreach (ModelMesh mesh in model.Meshes)
            {
                ...
                {
                    effect.World = absoluteBoneTransforms[mesh.ParentBone.Index] * modelTransform;              //creates world matrix by transforming the parent bone of the model
                    effect.EnableDefaultLighting();                                                             //enables lighting property
                    effect.AmbientLightColor = new Vector3(1.0f, 1.0f, 1.0f);                                   //brightens the models so they are easier to see
        ...
        {
            spriteBatch.Begin();                            //needed to draw
            string output = msg;                            //sets variable to passed parameter
            ...
            spriteBatch.End();                              //needed to draw. spriteBatch MUST end.
        }
        ...
        private void updateModels(float timeDelta)
        {
            ...
            {
                dfgList[i].Update(timeDelta);                       //performs update
            }

            for (int i = 0; i < GameConstants.NumBolts; i++)
            {
                if (boltList[i].isActive)                           //determines is a bolt is on screen
                {
                    boltList[i].Update(timeDelta);                  //if so it updates
                }
            }
            ...
            {
                case GameState.Boss:
                    for (int i = 0; i < GameConstants.NumBossBolts; i++)
                    {
                        if (bbList[i].isActive)                 //determines is a bolt is on screen
                        {
                            bbList[i].Update(timeDelta);        //if so it updates
                        }
                    }
                    break;
            }
        ...
        private void collisionCheck() //checks for a serious of collisions
        {
            ...
            BoundingSphere bossSphere = new BoundingSphere(bossPos,          //Bounding Sphere for the Boss
                mdlBoss.Meshes[0].BoundingSphere.Radius * 0.05f); 
                    ...
                    for (int k = 0; k < boltList.Length; k++)                               //loop for the bolts
                    {
                        if (boltList[k].isActive)                                           //determines if a bolt is active
                        {
                            ...
                            if (dfgSphereA.Intersects(boltSphere))                          //check for collision between a bolt and the DFG
                            {
                                ...
                                score += 10;
                                GamePad.SetVibration(PlayerIndex.One, .2f, .2f);            //low level vibration when there is a hit
                            ...
                            {
                                boltList[k].isActive = false;                               //removes the bolt
                                bossHealth = bossHealth - 5;                                //reduces the bosses health
                                bossHit.Play(0.2f, 0.0f, 0.0f);                             //plays a sound effect
                                score += 500;                                               //increases the score
                                GamePad.SetVibration(PlayerIndex.One, .5f, .5f);            //mid level vibration
                            }
                        ...
                        if (dfgSphereA.Intersects(ysSphere))                        //Check collision between dfg and the player
                        {
                            ...
                            totalCount++;                                           //increases total count
                            score--;                                                //reduces the score
                            GamePad.SetVibration(PlayerIndex.One, 0.0f, 0.3f);      //low-mid level vibration
                            break;                                                  //no need to check other DFG
                        }
                    }

                    for (int l = 0; l < bbList.Length; l++)                         //loop that checks collision for the boss bolts
                    {
                        if (bbList[l].isActive)                                     //determines if a  boss bolt is active
                        {
                            BoundingSphere bossboltSphere = new BoundingSphere(
                              bbList[l].position, mdlBolt.Meshes[0].BoundingSphere.Radius *
                                     GameConstants.BoltBoundingSphereScale);        //if so it creates a sphere for it

                            if (bossboltSphere.Intersects(ysSphere))                //actual check for intersection
                            {
                                pHealth = pHealth - 10;                             //as it is a boss it removes more health than the normal enemies
                                GamePad.SetVibration(PlayerIndex.One, 1.0f, 1.0f);
                                bbList[l].isActive = false;                         //deactivates the bolt
                            }
                        }
                    }
        ...
        private void updateBoss(float delta)                                        //updates the boss position
        {
            float speed = 8.0f * delta;                                             //sets speed
            int rand = random.Next(0, GameConstants.NumWaypoints);
            ...
            {
                n = rand;                                                           //sets n to random number so the bossknows what random waypoint to move toward
            ...
            for (int i = 0; i < GameConstants.NumBossBolts; i++)                //loop to create a bolt that the boss will fire.
            {
                if (!bbList[i].isActive)                                        //creates the bolts if there are none active in the game
                {
                    Matrix tardisTransform = Matrix.CreateRotationX(90);
                    bbList[i].direction = tardisTransform.Right;                //sets direction of the bolt, travels from right to left
                    bbList[i].speed = GameConstants.BossBoltSpeedAdjustment;     //sets the speed of the bolt
                    bbList[i].position = bossPos + bbList[i].direction;          //updates the position with the bolt direction
                    bbList[i].isActive = true;                                   //activates the bolts
                    boltFire.Play(0.1f, 0.0f, 0.0f);                            //plays SFX. low volume with default pan and pitch
                    break;                                                      //exit the loop     
                }
            }
        }
        ...
        protected override void LoadContent()
        {
            ...
            bossBolt = Content.Load<Model>(".\\Models\\laser");
            bossboltTransforms = SetupEffectTransformDefaults(mdlBolt); //bolt transform
        ...
        protected override void Update(GameTime gameTime)
        {
            ...
            gTimer -= gameTime.ElapsedGameTime.TotalSeconds;
            ...
            switch (CurrentState) //switch statement determines what is updated in what state
            {
                ...
                case GameState.Play: 
                    ...
                    if (pHealth <= 0 || gTimer <= 0)                                    //game ends if the player health is 0
                        CurrentState = GameState.Lose;

                    if (totalCount == dfgList.Length * 8)                               //checks to see if the total count is equal to 5 times the lengh of the dfgList
                    {
                ...
                case GameState.Boss:
                    ...
                    updateBoss(timeDelta);                      //updates the boss

                    collisionCheck();                           //check for collisions

                    if (pHealth <= 0 || gTimer <= 0)            //exit game is the player does
                        CurrentState = GameState.Lose;
                ...
                case GameState.Win:
                    ...
                    scoreFile = new StreamWriter(".//score.txt");                 //opens/creates the file to write to
                    scoreFile.WriteLine("Score: " + score + "\n");              //this is the text that is written to the file
                    scoreFile.Close();                                          //we need to close this file
        ...
        protected override void Draw(GameTime gameTime)
        {
            ...
            writeText("Remaining Time: " + ((int)gTimer).ToString(), new Vector2(550, 10), Color.Yellow);                               //remaining time
            writeText("Score: " + score.ToString(), new Vector2(40, 10), Color.Yellow);                                                 //player score
            writeText("Sound Toggled: " + soundToggle.ToString(), new Vector2(40, 50), Color.Yellow);                                   //whether or not the sound is toggled
            writeText("Free Camera: " + myCam.ToString(), new Vector2(550, 50), Color.Yellow);                                          //if the camera is set to free roam
            ...
            switch (CurrentState)
            {
                ...
                case GameState.Boss:
                    ...
                    for (int i = 0; i < GameConstants.NumBossBolts; i++)                //begins the loop for boss bolts
                    {
                        if (bbList[i].isActive)                                         //checks to see if they are active
                        {
                            Matrix bossboltTrans = Matrix.CreateScale(GameConstants.BoltScalar) * Matrix.CreateTranslation(bbList[i].position); //transforms each boss bolt
                            DrawModel(bossBolt, bossboltTrans, bossboltTransforms);     //draws each  boss bolt in the loop.
                        }
                    }
