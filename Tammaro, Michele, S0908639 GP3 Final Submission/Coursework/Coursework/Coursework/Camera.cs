...
namespace Lab5
{
    ...
    {
        ///www.youtube.com/watch?v=XkpZLzT5OV4
        //This class was implemented following the tutorial on the above link. Lines of code prefixed 
        //with the comment "//MGT" indicate changes I have made.
        ...
        private void Rotate(Vector3 scale) //MGT
        {
            MoveTo(Position, PreviewRotate(scale)); //MGT
        }

        private Vector3 PreviewRotate(Vector3 amount)//MGT
        {
            return amount + cameraRotation;//MGT
        }

        //Needed for camera manipulation using the GamePad
        GamePadState gps; //MGT
        ...
        public override void Update(GameTime gameTime)
        {
            ...
            gps = GamePad.GetState(PlayerIndex.One);                    //sets the state of the GamePad to gps //MGT
            ...
            if (rotateVector != Vector3.Zero)
            {
                ...
                rotateVector *= dt * cameraSpeed / 3; //MGT
                Rotate(rotateVector);
            }
            ...
            if (moveVector != Vector3.Zero) //MGT
            {
                //normalize that vector
                //so that we don't move faster diagonally
                moveVector.Normalize();     //MGT
                //Now we add in smooth and speed
                moveVector *= dt * cameraSpeed * 10;//MGT
